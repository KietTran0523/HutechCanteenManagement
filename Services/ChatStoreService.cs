...
WHERE r.[Id] = @Id;";
    ...
    public async Task<int> CreatePrivateRequestAsync(string requesterId, string receiverId)
    {
        if (string.IsNullOrWhiteSpace(requesterId) ||
            ...
            string.Equals(requesterId, receiverId, StringComparison.Ordinal))
        {
            throw new ArgumentException("Requester and receiver must be two different users.");
        }
...
BEGIN
    DELETE FROM [dbo].[ChatPrivateRequests] WHERE [RequesterId] = [ReceiverId];
...
IF NOT EXISTS (SELECT 1 FROM sys.check_constraints WHERE [name] = N'CK_ChatPrivateRequests_DifferentUsers')
    ALTER TABLE [dbo].[ChatPrivateRequests]
    ADD CONSTRAINT [CK_ChatPrivateRequests_DifferentUsers] CHECK ([RequesterId] <> [ReceiverId]);
